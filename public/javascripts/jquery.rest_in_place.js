...
RestInPlaceEditor.prototype = {
  ...
  abort : function() {
    this.element
      .html(this.oldValue).bind('click', {editor: this}, this.clickHandler);
  },
  
  update : function() {
    var editor = this;
		if (this.getValue() == "") {
		editor.element.html("<span style='color:red'>Atenció! Has esborrat el contingut. Si us plau, recarrega la pàgina i introdueix contingut nou.</span>")
		} else {
	    editor.ajax({
	      "type"       : "post",
	      "dataType"   : "text",
	      "data"       : this.requestData(),
			...
			// Això restableix el value del camp
  ...
  loadSuccessCallback : function(data) {
    ...
    this.element.html(data[this.objectName]);
		//Amb això torno a fer editable el camp després d'haver-lo editat un cop
    this.element.bind('click', {editor: this}, this.clickHandler);
  },
